# Lekcja: Triggery w SQL (MySQL) – 90 min

## 1. Cele lekcji (5 min)

Na tej lekcji uczniowie:

- zrozumieją, czym jest **trigger** i po co się go używa,
- poznają podstawową składnię triggera w MySQL,
- utworzą proste triggery: `BEFORE INSERT`, `AFTER INSERT`, `BEFORE UPDATE`,
- zobaczą, jak trigger pomaga w logowaniu zmian i automatycznym uzupełnianiu danych,
- przećwiczą diagnozowanie błędów (np. kiedy trigger blokuje wstawianie danych).
...
Wyjaśnij po kolei (wystarczy przeczytać):
...
Powiedz też o dwóch specjalnych słowach, które pojawią się w przykładach:
...
Możesz poprosić kilku uczniów, by odczytali na głos swój wiersz logu.
...
- poproś uczniów, aby sami napisali prosty opis do `log_zmian` w osobnym triggerze `AFTER UPDATE` (np. „Zmieniono stan magazynowy produktu …”).
...
Zamiast poprawiać dane, wyświetl błąd (nauczyciel może przeczytać gotową instrukcję):
